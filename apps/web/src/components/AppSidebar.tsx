...
type NavItem = { href: string; label: string; disabled?: false } | { label: string; disabled: true };
...
export function AppSidebar() {
  ...
  return (
    <>
      <aside className="w-56 bg-white border-r border-border flex flex-col shrink-0" aria-label="Navegación principal">
                  ...
                  if (item.disabled) {
                    return (
                      ...
                      >
                        ...
                        {item.label}
                        <span className="ml-auto text-[9px] bg-muted px-1.5 py-0.5 rounded font-medium">Pronto</span>
                      </span>
                    );
                  }
                  const active =
