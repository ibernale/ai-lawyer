...
class StubDataStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, sgAurora: ec2.ISecurityGroup, props: cdk.StackProps) {
    super(scope, id, props);
    ...
    const makeBucket = (bid: string): s3.IBucket =>
      s3.Bucket.fromBucketName(this, bid, `lex-agents-${bid.toLowerCase()}-dev-123`);
    this.buckets = {
      raw: makeBucket('Raw'),
...
function buildStack(): Template {
  ...
  const dataStack = new StubDataStack(app, 'TestData', networkStack.sgAurora, { env });
  const appServicesStack = new StubAppServicesStack(
    app, 'TestApp', networkStack.vpc, { env },
  );
...
describe('PipelineStack — State machine', () => {
  ...
  test('State machine definition contains all 8 expected states', () => {
    ...
    const flatten = (v: unknown): string => {
      ...
      if (v && typeof v === 'object') return Object.values(v as object).map(flatten).join('');
      return '';
    };
  ...
  test('Lambda stages have retry configuration', () => {
    const machines = template.findResources('AWS::StepFunctions::StateMachine');
    const defRaw = (Object.values(machines)[0] as any).Properties.DefinitionString;
    const flatten = (v: unknown): string => {
      if (typeof v === 'string') return v;
      if (Array.isArray(v)) return v.map(flatten).join('');
      if (v && typeof v === 'object') return Object.values(v as object).map(flatten).join('');
