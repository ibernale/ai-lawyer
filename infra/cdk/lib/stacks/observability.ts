...
export class ObservabilityStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: ObservabilityStackProps) {
    ...
    const pipelineFailedAlarm = new cloudwatch.Alarm(this, 'PipelineExecutionFailedAlarm', {
      alarmName: `lex-agents-${envName}-pipeline-execution-failed`,
      alarmDescription: 'Format change detected in one or more pipeline sources',
      metric: new cloudwatch.Metric({
    ...
    const secretAgeAlarm = new cloudwatch.Alarm(this, 'SecretAgeWarnAlarm', {
      ...
      alarmDescription: 'One or more secrets have not been rotated within the required period',
      metric: new cloudwatch.Metric({
    ...
    sgAlertRouter.addEgressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(443), 'HTTPS to Slack webhook');
...
`),
    ...
    const alarmFanoutRule = new events.Rule(this, 'AlarmFanoutRule', {
      ...
      description: 'Route CloudWatch ALARM state changes to alert-router Lambda',
      eventPattern: {
    ...
    NagSuppressions.addStackSuppressions(this, [
      ...
      {
        id: 'AwsSolutions-L1',
        reason: 'Alert router Lambda pinned to python3.12. Runtime upgrades tracked via CDK updates.',
      },
      {
        id: 'HIPAA.Security-LambdaInsideVPC',
        reason: 'Alert router Lambda is deployed inside VPC (PRIVATE_WITH_EGRESS) to reach internal endpoints.',
      },
      {
        id: 'HIPAA.Security-LambdaConcurrency',
        reason: 'Alert router Lambda is triggered infrequently by alarm state changes; reserved concurrency not required.',
      },
      {
        id: 'HIPAA.Security-LambdaDLQ',
        reason: 'Alert router is best-effort notification; failed Slack notifications do not require DLQ.',
      },
      {
        id: 'AwsSolutions-IAM5',
        reason: 'cloudwatch:PutMetricData requires wildcard resource per AWS API design.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'Inline policy on alert router Lambda for CloudWatch PutMetricData is minimal surface area.',
      },
      {
        id: 'AwsSolutions-IAM4',
        reason: 'AWSLambdaVPCAccessExecutionRole attached by CDK for VPC Lambda execution.',
      },
    ]);
  }
}
