    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'CDK deploy role requires broad Resource: * for CloudFormation and CDK bootstrap bucket/SSM/ECR access. Standard CDK deployment pattern. Role is assumable ONLY from a specific repo+branch via OIDC StringEquals conditions.',
      },
      {
        id: 'AwsSolutions-IAM4',
        reason: 'GitHub deploy role does not use AWS managed policies — only inline scoped action policies.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'GitHub deploy role uses inline policies scoped to specific CDK deployment actions. Inline policy provides tighter coupling between the role and its permissions than a standalone managed policy.',
      },
    ]);
  }
}
