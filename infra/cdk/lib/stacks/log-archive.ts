...
export class LogArchiveStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: LogArchiveStackProps) {
    ...
    this.trailBucket.addToResourcePolicy(
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        id: "AwsSolutions-S1",
        reason:
          "Server access logs for the log-archive bucket itself would create a circular dependency. The bucket is encrypted with KMS and all access is captured in the org CloudTrail.",
      },
      {
        id: "HIPAA.Security-S3BucketLoggingEnabled",
        reason:
          "This IS the logging bucket. Adding server access logs to itself is circular. All access captured via CloudTrail organization trail.",
      },
      {
        id: "HIPAA.Security-S3BucketReplicationEnabled",
        reason:
          "Cross-region replication to eu-west-1 is configured via AWS Backup policies post-deploy to avoid circular stack dependencies at synth time.",
      },
      {
        id: "HIPAA.Security-S3BucketVersioningEnabled",
        reason:
          "Athena results bucket holds transient query results with 90-day lifecycle expiry. Versioning not required; WORM Object Lock applied to the primary trail bucket.",
      },
      {
        id: "AwsSolutions-ATH1",
        reason:
          "Athena workgroup enforces SSE-KMS encryption on results. enforceWorkGroupConfiguration: true prevents override.",
      },
      {
        id: "AwsSolutions-IAM5",
        reason:
          "CloudTrail CW role requires :* suffix on log group ARN to create streams. This is the documented AWS pattern for CloudTrail-to-CloudWatch integration.",
      },
      {
        id: "HIPAA.Security-IAMNoInlinePolicy",
        reason:
          "CloudTrail CW Logs role uses an inline policy scoped to a single specific log group ARN. This is narrower than a managed policy and follows least-privilege.",
      },
    ]);
  }
}
