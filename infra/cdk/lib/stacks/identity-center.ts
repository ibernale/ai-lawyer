...
export class IdentityCenterStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: cdk.StackProps) {
    ...
    const instanceArn = this.node.tryGetContext('lexAgents:identityCenterInstanceArn')
      ?? 'PLACEHOLDER_IDENTITY_CENTER_INSTANCE_ARN';

    // ── Permission Sets ───────────────────────────────────────────────────
    const administratorPS = new sso.CfnPermissionSet(this, 'AdministratorAccessPS', {
      instanceArn,
      name: 'AdministratorAccess',
      description: 'Full administrator access. MFA + email approval required.',
    ...
    const dataAnalystPS = new sso.CfnPermissionSet(this, 'DataAnalystAccessPS', {
      instanceArn,
    ...
    const securityAuditPS = new sso.CfnPermissionSet(this, 'SecurityAuditAccessPS', {
      instanceArn,
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'SSO permission sets use AWS managed policies by design; custom inline policies are additive but the managed policies are required for the permission sets to function.',
      },
    ]);
  }
}
