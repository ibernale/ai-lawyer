...
export class LangfuseStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: LangfuseStackProps) {
    ...
    // ECS → Aurora
    sgLangfuseEcs.addEgressRule(sgLangfuseAurora, ec2.Port.tcp(5432), 'PostgreSQL to Langfuse Aurora');
    // ECS → internet (HTTPS for container registry etc.)
    sgLangfuseEcs.addEgressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(443), 'HTTPS outbound');

    // Aurora ingress from ECS
    sgLangfuseAurora.addIngressRule(sgLangfuseEcs, ec2.Port.tcp(5432), 'PostgreSQL from Langfuse ECS');
    ...
    sgLangfuseAlb.addEgressRule(sgLangfuseEcs, ec2.Port.tcp(3000), 'Langfuse web port');
    // ECS ingress from ALB
    sgLangfuseEcs.addIngressRule(sgLangfuseAlb, ec2.Port.tcp(3000), 'From Langfuse ALB');

    // ── Aurora Serverless v2 cluster for Langfuse ─────────────────────────────
    const langfuseSubnetGroup = new rds.SubnetGroup(this, 'LangfuseSubnetGroup', {
    ...
    const langfuseAuroraCluster = new rds.DatabaseCluster(this, 'LangfuseAurora', {
    ...
    const nextauthSecret = new secretsmanager.Secret(this, 'LangfuseNextauthSecret', {
    ...
    const encryptionKeySecret = new secretsmanager.Secret(this, 'LangfuseEncryptionKeySecret', {
    ...
    // ── ECS Task Definition ───────────────────────────────────────────────────
    const langfuseTaskDef = new ecs.FargateTaskDefinition(this, 'LangfuseWebTaskDef', {
    ...
    langfuseTaskDef.addContainer('langfuse', {
      ...
      secrets: {
        DATABASE_USERNAME: ecs.Secret.fromSecretsManager(dbMasterSecret, 'username'),
        DATABASE_PASSWORD: ecs.Secret.fromSecretsManager(dbMasterSecret, 'password'),
    ...
    langfuseTaskDef.addContainer('xray-daemon', {
      ...
      image: ecs.ContainerImage.fromRegistry('public.ecr.aws/xray-daemon/aws-xray-daemon:3.x'),
      logging,
    ...
    const langfuseTg = new elbv2.ApplicationTargetGroup(this, 'LangfuseTg', {
      ...
      targets: [
        langfuseService.loadBalancerTarget({ containerName: 'langfuse', containerPort: 3000 }),
      ],
      healthCheck: {
    ...
    const langfuseAcmCertArn = this.node.tryGetContext('lexAgents:langfuseAcmCertArn') as string | undefined;
    ...
    const cert = acm.Certificate.fromCertificateArn(this, 'LangfuseCert', langfuseAcmCertArn);
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'Langfuse image pulled from ghcr.io (not ECR). No ECR alternative exists for official Langfuse v3 image.',
      },
      {
        id: 'AwsSolutions-EC23',
        reason: 'ALB listener without custom ACM cert in dev. Set lexAgents:langfuseAcmCertArn context key to enable TLS.',
      },
      {
        id: 'HIPAA.Security-ECSTaskDefinitionUserForHostMode',
        reason: 'Langfuse container runs as default user (non-root enforcement is upstream responsibility of the ghcr.io image).',
      },
      {
        id: 'AwsSolutions-IAM5',
        reason: 'ECS task role wildcard permissions scoped to minimum required for CloudWatch Logs and X-Ray.',
      },
      {
        id: 'AwsSolutions-IAM4',
        reason: 'AmazonECSTaskExecutionRolePolicy is AWS-recommended for ECS task execution.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Langfuse uses password auth (not IAM) — iamAuthentication: false is intentional.',
      },
      {
        id: 'AwsSolutions-RDS16',
        reason: 'PostgreSQL logs exported to CloudWatch via cloudwatchLogsExports.',
      },
      {
        id: 'AwsSolutions-ELB2',
        reason: 'ALB access logs not required for internal Langfuse ALB in dev.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Container Insights not required per container in dev; cluster-level insight is sufficient.',
      },
      {
        id: 'AwsSolutions-SMG4',
        reason: 'Langfuse secrets (nextauth, salt, encryption-key) are manually populated; no automated rotation in dev.',
      },
      {
        id: 'HIPAA.Security-CloudWatchLogGroupEncrypted',
        reason: 'Langfuse log group in dev; KMS encryption added in staging/prod.',
      },
      {
        id: 'HIPAA.Security-SecretsManagerRotationEnabled',
        reason: 'Langfuse infrastructure secrets are static — rotation managed manually per ADR 0050.',
      },
      {
        id: 'HIPAA.Security-SecretsManagerUsingKMSKey',
        reason: 'Langfuse secrets use AWS managed KMS in dev; CMK added in prod.',
      },
      {
        id: 'HIPAA.Security-RDSLoggingEnabled',
        reason: 'Aurora PostgreSQL cloudwatchLogsExports includes postgresql log.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Deletion protection disabled intentionally for dev Langfuse cluster.',
      },
      {
        id: 'HIPAA.Security-RDSMultiAZSupport',
        reason: 'Aurora Serverless v2 writer provides equivalent resilience in dev.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'Inline policies on ECS roles are CDK-generated minimal-scope policies.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Dev: no ACM cert; provide lexAgents:langfuseAcmCertArn for HTTPS.',
      },
    ]);
  }
}
