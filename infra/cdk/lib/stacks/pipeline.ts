...
export class PipelineStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: PipelineStackProps) {
    ...
    [fetchRawFn, parseCanonicalFn, chunkDocumentFn, contextualizeChunksFn].forEach(fn => {
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'SFN role needs wildcard for ECS RunTask (cluster-scoped by condition), X-Ray, and CloudWatch Logs delivery APIs. Scoped to minimum required actions.',
      },
      {
      ...
      {
        id: 'AwsSolutions-SF1',
        reason: 'Step Functions logging level set to ERROR; full execution data logging omitted to avoid storing PII in logs.',
      },
      {
        id: 'AwsSolutions-SF2',
        reason: 'X-Ray tracing is enabled on the state machine (tracingConfiguration.enabled = true).',
      },
      {
        id: 'AwsSolutions-L1',
        reason: 'Pipeline Lambda functions are pinned to python3.12 (latest supported). Runtime upgrades tracked via CDK/dependency updates.',
      },
      {
        id: 'HIPAA.Security-CloudWatchLogGroupEncrypted',
        reason: 'SFN execution log group in dev; KMS encryption will be added in staging/prod.',
      },
      {
        id: 'HIPAA.Security-LambdaInsideVPC',
        reason: 'All pipeline Lambda functions are deployed inside the VPC (PRIVATE_WITH_EGRESS subnets).',
      },
      {
        id: 'HIPAA.Security-LambdaConcurrency',
        reason: 'Pipeline Lambdas are invoked sequentially by Step Functions; reserved concurrency not required.',
      },
      {
        id: 'HIPAA.Security-LambdaDLQ',
        reason: 'Step Functions handles retries and error routing; a Lambda DLQ would create duplicate error handling.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'Inline policies on SFN and scheduler roles provide minimal surface area; moved to managed policies in production.',
      },
    ]);
  }
}
