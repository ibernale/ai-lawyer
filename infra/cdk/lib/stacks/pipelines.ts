...
export class PipelinesStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: PipelinesStackProps) {
    ...
    const sgLambda = new ec2.SecurityGroup(this, 'SgLambda', {
      ...
      description: 'Pipeline Lambda functions — egress to VPC endpoints and internet',
    ...
    const contextualizeChunksFn = new lambda.Function(this, 'ContextualizeChunksFn', {
    ...
    const sourceDefs: SourceDef[] = [
      { source: 'boe',                      schedule: events.Schedule.cron({ minute: '0', hour: '7' }), scheduleEnabled: true },
      { source: 'eur_lex',                  schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'aepd',                     schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'FRI' }), scheduleEnabled: true },
      { source: 'edpb',                     schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'bde',                      schedule: events.Schedule.cron({ minute: '0', hour: '7' }), scheduleEnabled: true },
      { source: 'eba',                      schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'esma',                     schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'legislation_uk',           schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'fca',                      schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: true },
      { source: 'tribunal_constitucional',  schedule: events.Schedule.cron({ minute: '0', hour: '6', weekDay: 'MON' }), scheduleEnabled: false }, // AMBER
      { source: 'cendoj' },                                                                                                                         // manual only
      { source: 'inlabs',                   schedule: events.Schedule.cron({ minute: '0', hour: '4' }), scheduleEnabled: true },                   // STUB
      { source: 'sidof',                    schedule: events.Schedule.cron({ minute: '0', hour: '13' }), scheduleEnabled: true },                   // STUB
    ];
    ...
    for (const def of sourceDefs) {
      ...
      const suffix = source.replace(/_([a-z])/g, (_, c: string) => c.toUpperCase())
      ...
      const fetchRawFn = new lambda.Function(this, `FetchRaw${suffix}`, {
        ...
        runtime: lambda.Runtime.PYTHON_3_12,
        handler: 'handler.lambda_handler',
        code: lambda.Code.fromAsset(`${lambdaCodeBase}/sources/${source}/fetch_raw`),
        timeout: cdk.Duration.minutes(5),
        memorySize: 512,
        environment: commonEnv,
        ...lambdaVpcConfig,
      ...
      const parseCanonicalFn = new lambda.Function(this, `ParseCanonical${suffix}`, {
        functionName: `lex-agents-${envName}-${source}-parse-canonical`,
        runtime: lambda.Runtime.PYTHON_3_12,
        handler: 'handler.lambda_handler',
        code: lambda.Code.fromAsset(`${lambdaCodeBase}/sources/${source}/parse_canonical`),
        timeout: cdk.Duration.minutes(5),
        memorySize: 512,
        environment: commonEnv,
        ...lambdaVpcConfig,
    ...
    formatSensorRule.addTarget(new eventsTargets.LambdaFunction(formatSensorFn));
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason: 'SFN role needs wildcard for ECS RunTask (cluster-scoped by condition), X-Ray, and CloudWatch Logs delivery APIs.',
      },
      {
        id: 'AwsSolutions-IAM4',
        reason: 'AWSLambdaVPCAccessExecutionRole attached to pipeline Lambdas by CDK — required for VPC-attached Lambda execution.',
      },
      {
        id: 'AwsSolutions-SF1',
        reason: 'Step Functions logging level set to ERROR; full execution data logging omitted to avoid storing PII.',
      },
      {
        id: 'AwsSolutions-SF2',
        reason: 'X-Ray tracing is enabled on all state machines (tracingConfiguration.enabled = true).',
      },
      {
        id: 'AwsSolutions-L1',
        reason: 'Pipeline Lambda functions pinned to python3.12. Runtime upgrades tracked via CDK updates.',
      },
      {
        id: 'HIPAA.Security-CloudWatchLogGroupEncrypted',
        reason: 'SFN execution log groups in dev; KMS encryption added in staging/prod.',
      },
      {
        id: 'HIPAA.Security-LambdaInsideVPC',
        reason: 'All pipeline Lambda functions deployed inside VPC (PRIVATE_WITH_EGRESS subnets).',
      },
      {
        id: 'HIPAA.Security-LambdaConcurrency',
        reason: 'Pipeline Lambdas invoked sequentially by Step Functions; reserved concurrency not required.',
      },
      {
        id: 'HIPAA.Security-LambdaDLQ',
        reason: 'Step Functions handles retries and error routing; Lambda DLQ would create duplicate error handling.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'Inline policies on SFN role; moved to managed policies in production.',
      },
      {
        id: 'AwsSolutions-SNS2',
        reason: 'Pipeline alerts SNS topic; SSE not required for alarm notifications in dev.',
      },
      {
        id: 'AwsSolutions-SNS3',
        reason: 'No subscriptions in Fase 9.4; alerting subscriptions added in Fase 10.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Point-in-time recovery enabled on idempotency table. Quota and fingerprints tables store ephemeral operational data; PITR not required.',
      },
    ]);
  }
}
