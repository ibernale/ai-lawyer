...
export class NetworkHubStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: cdk.StackProps) {
    ...
    const hubSubnets =
      this.hubVpc.isolatedSubnets.length > 0
    ...
    resolverSg.addIngressRule(
      ...
      ec2.Port.udp(53),
      "DNS from private ranges",
    );
    resolverSg.addIngressRule(
      ec2.Peer.ipv4("10.0.0.0/8"),
      ec2.Port.tcp(53),
      "DNS TCP from private ranges",
    );
    ...
    NagSuppressions.addStackSuppressions(this, [
      {
        ...
        reason:
          "VPC Flow Logs for the hub VPC are configured post-deploy via AWS CLI to the log-archive S3 bucket. Cross-account reference at synth time would create circular dependencies.",
      },
      {
        id: "HIPAA.Security-VPCFlowLogsEnabled",
        reason:
          "Flow logs configured post-deploy to log-archive S3 bucket via CLI to avoid cross-account synth-time dependencies.",
      },
      {
        id: "HIPAA.Security-VPCDefaultSecurityGroupClosed",
        reason:
          "Hub VPC default SG is not used by any workload. All hub resources use dedicated security groups.",
      },
    ]);
  }
}
