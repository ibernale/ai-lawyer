...
export class ComplianceStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: ComplianceStackProps) {
    ...
    new cloudwatch.Alarm(this, 'ComplianceScoreAlarm', {
      ...
      alarmDescription: 'DORA compliance score dropped below 90%. Investigate evidence_collector reports.',
      metric: scoreMetric,
      threshold: 90,
    ...
    NagSuppressions.addResourceSuppressions(fnRole, [
      {
        id: 'AwsSolutions-IAM4',
        reason: 'AWSLambdaBasicExecutionRole is the minimal managed policy for Lambda logging.',
      },
      {
        id: 'AwsSolutions-IAM5',
        reason: 'Evidence collector needs read access to KMS, GuardDuty, Config across all resources in the account.',
      },
    ], true);
    NagSuppressions.addResourceSuppressions(this.complianceBucket, [{
      id: 'AwsSolutions-S1',
      reason: 'Server access logs not enabled: compliance bucket is itself the audit sink.',
    }]);
    NagSuppressions.addStackSuppressions(this, [
      {
        id: 'HIPAA.Security-S3BucketLoggingEnabled',
        reason: 'Compliance bucket is an audit sink; enabling server access logging would be circular.',
      },
      {
      ...
      },
      {
        ...
        reason: 'Evidence collector Lambda calls AWS service APIs via HTTPS; no VPC placement required.',
      },
      {
        id: 'HIPAA.Security-LambdaConcurrency',
        reason: 'Evidence collector runs once daily; reserved concurrency not warranted.',
      },
      {
        id: 'HIPAA.Security-LambdaDLQ',
        reason: 'Evidence collector is scheduled daily with CloudWatch Alarm on score drop; DLQ not required.',
      },
      {
        id: 'HIPAA.Security-IAMNoInlinePolicy',
        reason: 'Inline AuditReadAccess policy is narrowly scoped to read-only audit APIs.',
      },
      {
        id: 'HIPAA.Security-CloudWatchLogGroupEncrypted',
        reason: 'Evidence collector log group is encrypted with the KMS logsKey.',
      },
    ]);
  }
}
