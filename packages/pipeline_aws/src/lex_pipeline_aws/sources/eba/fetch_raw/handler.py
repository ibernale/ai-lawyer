"""fetch_raw handler for EBA — Lambda step in ingest_eba state machine.

Fetches EBA (European Banking Authority) Single Rulebook Q&A documents (HTML),
trying the JSON API first then falling back to HTML scraping.
Stores raw bytes in the S3 raw bucket.

EbaSource.list_documents() takes no arguments; it returns IDs in the format
"EBA-QA-<number>".
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """Step Functions Lambda handler for the EBA FetchRaw stage."""
    evt = PipelineEvent.from_dict(event)
    logger.info("eba.fetch_raw_start", run_date=evt.run_date)

    try:
        documents = _run_async(_fetch_documents())
    except Exception as exc:
        logger.error("eba.fetch_raw.list_failed", error=str(exc))
        evt.status = "error"
    ...
    for doc_id, raw_bytes, content_type in documents:
        ...
        s3_key = f"eba/{evt.run_date}/{safe_id}.html"
        pipeline_doc_id = f"eba/{evt.run_date}/{safe_id}"

        try:
            acquire_lock(pipeline_doc_id, "fetch_raw")
        except AlreadySucceeded as exc:
            logger.info("eba.fetch_raw.cached", doc_id=pipeline_doc_id)
            stored_keys.append(exc.cached_output.get("raw_s3_key", ""))
            continue
        except StillRunning:
            logger.warning("eba.fetch_raw.still_running", doc_id=pipeline_doc_id)
            continue
        ...
        try:
            ...
            logger.info("eba.fetch_raw.uploaded", doc_id=pipeline_doc_id, s3_key=s3_key)
        except Exception as exc:
            fail_lock(pipeline_doc_id, "fetch_raw", str(exc))
            failed_ids.append(pipeline_doc_id)
            logger.error("eba.fetch_raw.upload_failed", doc_id=pipeline_doc_id, error=str(exc))

    last_doc_id = f"eba/{evt.run_date}/batch"
    ...
    output = {
        ...
        "eba_docs_stored": len(stored_keys),
        "eba_docs_failed": len(failed_ids),
    }
