"""parse_canonical handler for EBA — Lambda step in ingest_eba state machine.

Reads a raw EBA Q&A HTML document from S3, parses it into canonical JSON
using EbaSource.parse_to_canonical(), and stores in the canonical S3 bucket.
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    """Step Functions Lambda handler for the EBA ParseCanonical stage."""
    ...
    logger.info("eba.parse_canonical_start", doc_id=evt.doc_id)

    try:
        try:
            acquire_lock(evt.doc_id, "parse_canonical")
        except AlreadySucceeded as exc:
            logger.info("eba.parse_canonical_cached", doc_id=evt.doc_id)
        ...
        source = EbaSource.__new__(EbaSource)
        ...
        s3_key = f"eba/{evt.run_date}/{safe_doc_id}.json"
        s3 = boto3.client("s3")
        s3.put_object(
            ...
            Metadata={"doc_id": evt.doc_id, "source": "eba"},
        )

        logger.info("eba.parse_canonical_done", doc_id=evt.doc_id, s3_key=s3_key)
        ...
        logger.error("eba.parse_canonical_error", error=str(exc), doc_id=evt.doc_id)
