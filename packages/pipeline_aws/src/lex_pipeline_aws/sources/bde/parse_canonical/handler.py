"""parse_canonical handler for BdE — Lambda step in ingest_bde state machine.

Reads a raw BdE Circular HTML document from S3, parses it into canonical JSON
using BdeSource.parse_to_canonical(), and stores in the canonical S3 bucket.
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        try:
            ...
            logger.info("bde.parse_canonical_cached", doc_id=evt.doc_id)
        ...
        # doc_id format: "bde/2026-05-14/BDE-CIRC-5-2024"
        # source_id for BdeSource is the last segment
        ...
        source = BdeSource.__new__(BdeSource)
        ...
        s3_key = f"bde/{evt.run_date}/{safe_doc_id}.json"
        s3 = boto3.client("s3")
        s3.put_object(
            ...
            Metadata={"doc_id": evt.doc_id, "source": "bde"},
        )

        logger.info("bde.parse_canonical_done", doc_id=evt.doc_id, s3_key=s3_key)
        ...
        logger.error("bde.parse_canonical_error", error=str(exc), doc_id=evt.doc_id)
