"""parse_canonical handler for FCA — Lambda step in ingest_fca state machine.

Reads a raw FCA Handbook HTML document from S3, parses it into canonical JSON
using FcaSource.parse_to_canonical(), and stores in the canonical S3 bucket.

Note: FcaSource.parse_to_canonical() reads _section_meta to resolve section
titles. We reconstruct the minimal state needed: section_meta dict seeded from
COVERED_SECTIONS constant.
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    try:
        try:
            ...
            logger.info("fca.parse_canonical_cached", doc_id=evt.doc_id)
        ...
        # Restore slash in chapter IDs: "PRIN_1" → "PRIN/1"
        source_id_restored = source_id.replace("_", "/", 1) if "_" in source_id else source_id

        raw_doc = RawDocument(
            source="fca",
            source_id=source_id_restored,
        ...
        # Reconstruct minimal source state for parse_to_canonical
        source = FcaSource.__new__(FcaSource)
        source._section_meta = {code: title for code, _, title in COVERED_SECTIONS}
        ...
        s3_key = f"fca/{evt.run_date}/{safe_doc_id}.json"
        s3 = boto3.client("s3")
        s3.put_object(
            ...
            Metadata={"doc_id": evt.doc_id, "source": "fca"},
        )

        logger.info("fca.parse_canonical_done", doc_id=evt.doc_id, s3_key=s3_key)
        ...
        logger.error("fca.parse_canonical_error", error=str(exc), doc_id=evt.doc_id)
